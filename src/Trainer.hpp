...
struct Trainer: public DataExporter {
  ...
  // functions
  Trainer(
      ostream& o, Mdrnn* n, ConfigFile& conf, const string& name = "trainer"):
      DataExporter(name), out(o), net(n), optimiser(0), config(conf),
    ...
    if (optType == "rprop") {
      ...
      optimiser = new SteepestDescent(
          optName, out, wts, derivs, learnRate, momentum);
    }
    if (mdl) {
      ...
      if (mdlOptType == "rprop") {
        ...
        mdlOptimiser = new SteepestDescent(
            mdlOptName, out, mdlStdDevs, mdlStdDevDerivs,

...
struct Mdrnn {
  ...
  Mdrnn(ostream& o, ConfigFile& conf, const DataHeader& data):
      out(o), inputLayer(new InputLayer(
          "input", data.numDims, data.inputSize, data.inputLabels)),
      ...
      inputBlockLayer(in(inputBlock, 0) ? 0 : add_layer(
          new BlockLayer(inputLayer, inputBlock), false))
  { }
  ...
  int copy_connections(Layer* src, Layer* dest, bool mirror = false) {
    ...
    LOOP(PLC c, connections.equal_range(src)) {
      ...
      add_connection(new FullConnection(oldConn->from == oldConn->to ? dest :
                                        oldConn->from, dest, delay, oldConn));
      ++numCopied;
    }
    return numCopied;
  }

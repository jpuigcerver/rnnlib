...
struct TranscriptionLayer: public SoftmaxLayer, public NetworkOutput {
  ...
  // functions
  TranscriptionLayer(
      ostream& o, const string& name, const vector<string>& labs,
      bool cm = false) :
  ...
  real_t calculate_errors(const DataSequence& seq) {
    ...
    if (totalTime < requiredTime) {
      out << "warning, seq " << seq.tag << " has requiredTime "
          << requiredTime << " > totalTime " << totalTime << endl;
      return 0;
    }
    ...
    check(
        logProb.log() <= 0.0 || fabs(logProb.log() - 0.0) <= 1E-9,
        "sequence\n" + str(seq) + "has log probability " + str(logProb.log()));
    
    // calculate the backward variables
    backwardVariables.reshape_with_depth(
        list_of(totalTime), totalSegments, Log<real_t>());
    ...
    LOOP_BACK(int t, span(totalTime - 1)) {
      ...
      LOOP(int s, segment_range(t)) {
        ...
        if (s&1) {  // s odd (label output)
          int labelIndex = s/2;
          int labelNum = seq.targetLabelSeq[labelIndex];
          bv = (oldBvars[s] * oldLogActs[labelNum] * prior_label_prob(
              labelIndex)) + (oldBvars[s + 1] * oldLogActs[blank]);
    ...
    if (confusionMatrix) {
      ...
      LOOP (const subsPair& p, alignment.subsMap) {
        ...
        LOOP (const PII& p2, p.second) {
          errorMap["_" + targetLabels[refIndex] + "->" +
                   targetLabels[p2.first]] += (real_t)p2.second / normFactor;
        }
      }
    }
    if (verbose) {
      out << "target label sequence (length "
          << seq.targetLabelSeq.size() << "):" << endl
          << label_seq_to_str(seq.targetLabelSeq, targetLabels) << endl;
      out << "output label sequence (length "
          << outputLabelSeq.size() << ", timesteps " << totalTime << "):"
          << endl << label_seq_to_str(outputLabelSeq, targetLabels) << endl;
    }
    return ctcError;
  }
};

...
int main(int argc, char* argv[]) {
  ...
  LOOP(int arg, span(1, argc - 1)) {
    ...
    check(
        argument[0].substr(0, 2) == "--", "invalid option name " + argument[0]);
  ...
  check(
      dataFiles.size() > dataFileNum, "no " + ordinal(dataFileNum) +
      " file in size " + str(dataFiles.size()) + " file list " +
      dataFileString + " in " + configFilename);
  ...
  out << "setting random seed to " << Random::set_seed(
      conf.get<unsigned long int>("randSeed", 0)) << endl << endl;
  ...
  if (gradCheck)  {
    ...
    prt_line(out);
    GradientCheck(
        out, net, *testSeq, conf.get<int>("sigFigs", 6),
    ...
    out << endl;
    out << "data header:" << endl << header << endl;
    out << "calculating Jacobian for sequence "
        << displaySequence << " at coords " << jacobianCoords << endl;
    out << *testSeq;
    ...
    check(
        (jacobianCoords.size() == D) || (jacobianCoords.size() == (D - 1)),
